...
def fetch_data_cbex(data):
    while True:
        ...
        for bid in bids_cbex:
            ...
            if bid[0] > data["best_bid_cbex"]:
                data["best_bid_cbex"] = bid[0]

        for ask in asks_cbex:
            print("Asks_cbex: ",asks_cbex)
            if ask[0] < data["best_ask_cbex"]:
                data["best_ask_cbex"] = ask[0]

def fetch_data_okx(data):
    while True:
        if datetime.datetime.now() >= endTime:
            break
        ...
        for bid in bids_okx:
            ...
            if bid[0] > data["best_bid_okx"]:
                data["best_bid_okx"] = bid[0]

        for ask in asks_okx:
            print("asks_okx: ",asks_okx)
            if ask[0] < data["best_ask_okx"]:
                data["best_ask_okx"] = ask[0]
...
if __name__ == '__main__':
    ...
    cbex_process = Process(target=fetch_data_cbex, args=(data,))
    okx_process = Process(target=fetch_data_okx, args=(data,))
    ...
    best_bid_cbex = data["best_bid_cbex"]
    best_ask_cbex = data["best_ask_cbex"]
    best_bid_okx = data["best_bid_okx"]
    best_ask_okx = data["best_ask_okx"]

    print("------------------------------------------------------------")
    print("best_bid_cbex: ",data["best_bid_cbex"])
    print("best_ask_cbex: ",data["best_ask_cbex"])
    print("best_bid_okx: ",data["best_bid_okx"])
